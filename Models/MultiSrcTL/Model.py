...
class CustomMultiSrcTL():
    ...
    def empirical_error(self, src_i, src_j):
        net_i = self.src_domain[src_i]['model']
        net_j = self.src_domain[src_j]['model']
        
        input_data_i = self.src_domain[src_i]['data'][0]
        input_data_j = self.src_domain[src_j]['data'][0]

        y_pred = net_j(torch.Tensor(input_data_i)).detach().numpy()
        y_true = self.src_domain[src_i]['data'][1]
